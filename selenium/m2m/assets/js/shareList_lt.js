...
ss.imports(['dataTable'], function(exports) {
	...
	var instance = ss.dataTable({ //64,369
		...
		table: {
			...
			options: {
				...
				showTitle: ['pfPoolName', 'customerName', 'supplierName',
					'packageName', 'totalCardCount',
					'effectCount', 'availableQuantity', 'usedQuantity',
					'surplusQuantity', 'imsi', 'imei', 'createTime'
				],
				sort: {},
				shim: {
					'poolLimitType': {
			...
			tlName: ['共享池名称', '客户名称', '供应商', '运营商共享池', '套餐名称',
				'总卡数（张）', '生效卡数（张）', '可用量（G）', '已用量（G）',
				'剩余用量（G）', '扩容可用量（G）', '扩容已用量（G）', '创建日期', '池超量策略', '备注'
			], //表头名字
			tlTxt: ['pfPoolName', 'customerName', 'supplierName', 'poolOperatorName', 'packageName',
				'totalCardCount', 'effectCount', 'availableQuantity',
				'usedQuantity', 'surplusQuantity', 'imsi', 'imei', 'createTime', 'poolLimitType', 'remark'
			], //表头字段
			//操作项
			operation: [
				//编辑和删除为默认，其它按钮需txt
	...
	function modifyBag(self) {
		...
		//请求获取套餐
		function getPackage(params, type) {
			$.ajax({
				type: 'POST',
				...
				success: function(datajson) {
					if(datajson.result == 'success') {
						...
						parentDom.querySelector('#perLimit').value = 11;
						parentDom.querySelector('#buyNum').value = 11;
						parentDom.querySelector('#price').value = 11;
						bagData = data;
						var selectData = [];
						for(var i = 0; i < data.length; i++) {
							var obj = {};
						...
						layer.msg(datajson.errorMsg || '获取套餐数据失败')
					}
				},
		...
		parentDom.querySelector('#xufeiSave').onclick = function() {
			var renewType = $('input[type=radio][name=renewType]').val();
			...
			$.ajax({
				...
				success: function(data) {
					if(data.result == 'success') {
						...
						$('input[type=radio][name="payType"]:first').prop("checked", true);
	...
	function continues(self) {
					...
					if(selectLi && selectLi.length > 0) {
						for(var i = 0; i < selectLi.length; i++) {
							selectLi[i].onclick = function() {
								...
								var params = {
									customerUuid: customerUuid,
									bagType: $('input[type=radio][name=bagType]').val() || ''
								}
								getPackage(params)
							}
						}
		...
		//确定按钮
		parentDom.querySelector('#xufeiSave').onclick = function() {
			var effectType = $('input[type=radio][name=effectType]').val();
			var renewType = $('input[type=radio][name=renewType]').val();
			var bagType = $('input[type=radio][name=bagType]').val();
			var uuid = parentDom.querySelector('#poolName').querySelector("._show").getAttribute('code') || ''
			var singleBagCode = parentDom.querySelector('#packageName').querySelector("._show").getAttribute('code') || ''
			var params = {
				effectType,
