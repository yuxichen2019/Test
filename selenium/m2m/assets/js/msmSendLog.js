ss.imports(['dataTable'], function(exports) {
	...
	var instance = ss.dataTable({ //64,369
		...
		//数据选项
		dataOption: {
			listUrl: commonUrl + '/admin/TSmsSend/smsSendLog/queryByPageInfo.action', //请求Url
			type: 'post', //默认post请求
			dataType: "json",
		...
		searchBtn: {
			...
			reflesh: {
				...
				cbFn: function(curData, self) {
					$.ajax({
						type: 'POST',
						url: commonUrl + '/admin/TSmsSend/smsSendLog/refreshSMS.action',
		...
		table: {
			...
			tlName: [
				'接收号码', 'iccid', '所属客户', '短信内容',
				'短信状态', '发送时间', '接收时间', '创建人'
			], //表头名字
			tlTxt: [
				'sentTo', 'iccid', 'customerName', 'smsContent',
				'smsStatus', 'sentTime', 'receivedTime', 'senderName'
			], //表头字段
			//操作项
			operation: [
				//编辑和删除为默认，其它按钮需txt
	...
	function smsSendFn() {
		...
		$.ajax({
			type: 'POST',
			url: commonUrl + '/admin/TSmsModel/queryByPageInfo.action',
			data: JSON.stringify({
				currentPage: 1,
				pageSize: 10000
			}),
		...
		parentDom.querySelector("#stateSave").onclick = function() {
			console.log(parentDom.querySelector("#sentTo").value)
			console.log(parentDom.querySelector("._show").getAttribute('code'))
			console.log(parentDom.querySelector("#sentContent").value)
			$.ajax({
				type: 'POST',
				url: commonUrl + '/admin/TCardStore/smsStatusChange.action',
				data: JSON.stringify({
					sendToArr: parentDom.querySelector("#sentTo").value,
					msmModel: parentDom.querySelector("._show").getAttribute('code'),
