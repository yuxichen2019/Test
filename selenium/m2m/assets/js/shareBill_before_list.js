...
ss.imports(['dataTable'], function(exports) {
	...
	function getCustormer() {
		...
		$.ajax({
			...
			data: JSON.stringify({pageSize:10000}),
	...
	for(var a in curAttrObj) {
		...
		//客户数据回显
		if(a == 'customerUuid') {
			var customrData = getCustormer() || [];
			for(var i = 0; i < customrData.length; i++) {
				if(customrData[i].uuid == curObj[a]) {
					curObj[a] = customrData[i].customerName;
				}
			}
		}
		//共享池数据回显
		if(a == 'poolUuid') {
			var customrData = getPool() || [];
			for(var i = 0; i < customrData.length; i++) {
				if(customrData[i].uuid == curObj[a]) {
					curObj[a] = customrData[i].pfPoolName;
				}
			}
		}
		if(a == 'supId'){
			curObj[a] = operatorObj[curObj[a]]
		}
	...
	//主套餐（预）
	var tlTxtB = ['iccid', 'chargeTime', 'pfPoolName', 'oldBagName', 'oldFee', 'newFee', 'usedFlow']
	//主套餐（后）
	var tlTxtA = ['iccid', 'effectTime', 'pfPoolName', 'oldBagName', 'oldFee', 'isEditBag', 'newBagName', 'newFee', 'usedFlow']
	//扩容包
	var tlName2 = ['共享池名称', '扩容包名称', '充值时间', '计费金额(元)', '扩容包可用量(M)', '扩容包已用量(M)']
	var tlTxt2 = ['pfPoolName', 'bagCustomerName', 'chargeTime', 'fee', 'flowLimit', 'usedFlow']
	var curUrl1 = '';
	var curUrl2 = '';
	var isChangeTime = [];
	var dpWPer = '120%';
	...
	if(curObj['billType'] == 1) {
		...
		tlName = tlNameB;
		tlTxt = tlTxtB;
		isChangeTime = ['chargeTime']
	} else {
		curUrl1 = curUrlAM;
		curUrl2 = curUrlAE;
		tlName = tlNameA;
		tlTxt = tlTxtA;
		isChangeTime = ['effectTime'];
		dpWith = {
			'iccid':9, 
			'effectTime':8,
			'pfPoolName':7, 
	...
	var instance = ss.dataTable({ //64,369
		...
		dataOption: {
			...
			data: JSON.stringify({poolUuid:poolUuid,calculateCycle:curObj.calculateCycle}),
	...
	document.querySelector('#dataCon_gsz').querySelector('.unicom').onclick = function() {
		...
		instance.sourceObj.searchOption = searchOption2;
		instance.sourceObj.table.tlName = tlName2;
		instance.sourceObj.table.tlTxt = tlTxt2;
		instance.sourceObj.table.options.dpWPer = '';
		instance.sourceObj.table.options.dpWith = {};
		instance.sourceObj.table.options.isChangeTime = ['chargeTime'];
	...
	document.querySelector('#dataCon_gsz').querySelector('.mobile').onclick = function() {
		...
		instance.sourceObj.searchOption = searchOption;
		instance.sourceObj.table.tlName = tlName;
		instance.sourceObj.table.tlTxt = tlTxt;
		instance.sourceObj.table.options.dpWPer = '120%';
		instance.sourceObj.table.options.dpWith = dpWith;
		instance.sourceObj.table.options.isChangeTime = isChangeTime;
		instance.rd_searchFn();
		instance.lg_reloadFn();
	}
