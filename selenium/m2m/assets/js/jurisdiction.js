ss.imports(['dataTable'],function(exports) {
    ...
    View.prototype = {
                            ...
                            ss.crtDom('div','',addViewData[x].placeholder || addViewData[x].name,dom,{
                                ...
                                .appendDom([
                                    ...
                                    ss.crtDom('div','selectItems','','',{
                                        cn:[
                                        	'width',
                                        	'height','border','position','top','left','backgroundColor','borderRadius','overflowX','overflowY','display','zIndex'
                                    	],
                                        cv:[
                                        	addViewData[x].width?addViewData[x].width:'100%',
                                        	addViewData[x].data.length<5?'auto':7*30+'px','1px solid #ccc','absolute','32px','-1px','#fff','3px','hidden','auto','none',13
                                    	]
                                    })
                                        .appendDom(function(dom){
        ...
        lg_seaViewFn:function(paraObj,curData){
            ...
            //渲染数据
            //权限列表的数据
            var promissData = permissionInstance.scope.listData.permission.data.data;
            var shimObj ={};
            if(promissData && promissData.length>0){
            	promissData.forEach(function(item){
            		shimObj[item.uuid] = item.permissionName;
            	});
            }; 
                        ...
                        .appendDom(function(dom){
                            ss.crtDom(
                        		...
                        		(
                        			x=='platformType'?
                        			(
                        				endSeaData[x]==1?'管理端':'客户端'	
                        			)
                        				:
                    				(x=='parentUuid'&&endSeaData[x] ?shimObj[endSeaData[x]]:endSeaData[x])
                    			)
                            		:
                        		'-',
                            	dom,
    ...
    Jurisdiction.prototype = {
        ...
        lg_dpDataFn:function(){
            ...
            for(var s in self['scope']['listData']){
                ...
                for(var u=0; u<curData.length; u++){
                	if(s=='permission' || s=='source'){ 
                		var _index = curData[u].platformType==1?0:1;
	                    tempArr[_index]['data'].push({
        ...
        rd_layoutFn:function(){
            ...
            ['permission','source'].forEach(function(v,i){
                ...
                var item = ss.crtDom('div',v,'',tempAppendToDom,{
                    ...
                    cv:['inline-block',(v=='source'||v=='permission')?'40%':'300px','top','relative','5px']
                });
                self.domWrap[v] = item;
            });
        },
	...
	//新增下拉框渲染
	function crtDomFn(dataArr, sData ,dom ,type,self) {
		dom.innerHTML = '';
	...
	//菜单选项 _m2m_customer_heeler
	function getMenuCodesFn(mnData){
		var menuData = mnData;
		var _endArr = [];
            ...
            permission:{
                ...
                items:{
					...
					platformType:{
						...
						type:'select',
						verify:true,
						data:[{name:'管理端',code:'1'},{name:'客户端',code:'2'}],
						cbFn:function(dom,self){
							var _endStatu = dom.innerHTML=='管理端'?'1':(
												dom.innerHTML=='客户端'?'2':'3'
											);
							var mnObj = {
								'1':getMenuCodesFn(ss.options.menuData.heeler),
								'2':getMenuCodesFn(_m2m_customer_heeler),
								'3':getMenuCodesFn(ss.options.menuData.heeler).concat(getMenuCodesFn(_m2m_customer_heeler))
							};
							var _endLoadData = mnObj[_endStatu];
							
							var conDom = dom.parentNode.parentNode.parentNode.parentNode.parentNode; 
							var permissionLabelDom = conDom.querySelectorAll('[name="permissionLabel"]')[1];
							var pPDom = permissionLabelDom.querySelector('.selectItems');
							
							crtDomFn(
								[{name:'菜单模块',code:''}].concat(_endLoadData),
								'',
								pPDom,
								'add',
								self
							...
							//对已选的菜单模块进行重置
							ss.setDomTxt(permissionLabelDom,'菜单模块');
							ss.mdfCss(permissionLabelDom,['color','rgb(117, 117, 117)']);
							permissionLabelDom.setAttribute('code','');
							self['scope']['addParaObj']['permissionLabel'] = '';		
						}
					},
					permissionLabel:{
						name:'菜单模块',
						type:'select',
						data:getMenuCodesFn(ss.options.menuData.heeler).concat(
							getMenuCodesFn(_m2m_customer_heeler)
						),
						verify:true
					},
                    sourceCode:{name:'资源标签',type:'txt',verify:true},
                    remark:{name:'描述',type:'txt'},
                }
            },
            source:{
                ...
                items:{
                    ...
                    parantName:{
                    	name:'',type:'txt',isShow:'false'
                    },
					platformType:{
						name:'平台类型',type:'select',verify:true,
						data:[{name:'管理端',code:'1'},{name:'客户端',code:'2'}],
						cbFn:function(dom,self){
							var _index = dom.innerHTML == '管理端'?1:2;
		                    permissionInstance.eAjax(
		                        {
		                        	url:commonUrl+'/admin/permission/queryPermissionDataByPage.action',
		                        	type:'post',
		                        	data:{
		                        		pageSize:10000,
		                        		platformType:_index
		                        ...
		                        {
		                            success:function(data){
		                            	var endData = data.data.data;
		                            	var _dpArr = [];
		                            	endData.forEach(function(item){
		                            		_dpArr.push({
		                            			name:item.permissionName,code:item.uuid
		                            		})
		                            	});
		                            	
        								var conDom = dom.parentNode.parentNode.parentNode.parentNode.parentNode; 
										var permissionLabelDom = conDom.querySelectorAll('[name="parentUuid"]')[1];
										var pPDom = permissionLabelDom.querySelector('.selectItems');
										
										crtDomFn(
											[{name:'所属权限',code:''}].concat(_dpArr),
											'',
											pPDom,
											'add',
											self
										...
										//对已选的菜单模块进行重置
										ss.setDomTxt(permissionLabelDom,'所属权限');
										ss.mdfCss(permissionLabelDom,['color','rgb(117, 117, 117)']);
										permissionLabelDom.setAttribute('code','');
										self['scope']['addParaObj']['permissionLabel'] = '';	
		                            },
                    ...
                    parentUuid:{
                    	name:'所属权限',type:'select',verify:true,
						data:{
							url:commonUrl + '/admin/permission/queryPermissionDataByPage.action',
							dataType:'json',
							data:{
						...
						cbFn:function(dom,self){
							self['scope']['addParaObj'].parantName = dom.innerHTML;
						}
                    },
                    remark:{name:'描述',type:'txt'}
                }
...
//	ss.polling({
//		condition:function(){
//			return ss.getDom('#listWrap').querySelector('.permission')
//		},
//		endTimeout:10000,
//		cbFn:function(){
//			window.setTimeout(function(){
//				var cueListDom = ss.getDom('#listWrap');
//				ss.mdfCss(
//					cueListDom.querySelector('.user'),['display','none']
//				);
//				ss.mdfCss(
//					cueListDom.querySelector('.role'),['display','none']
//				);
//			},10)
//		}
//	})
