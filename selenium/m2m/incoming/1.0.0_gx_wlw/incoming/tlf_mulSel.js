(function(win){
    function tlf_mulSel(obj){
        this.sourceObj = obj;
        this.domWrap = {};//dom存储容器
        this.scope = {};//数据存储容器
        this.init();//初始化
    };
    tlf_mulSel.prototype = {
        constructor:tlf_mulSel,//构造函数
        //初始化
        init:function(){
            var self = this,obj = self.sourceObj;
            //对权限数据进行存储
            self.scope['endData'] = obj.data;
                        ...
                        .appendDom(function(dom){
                            ...
                            .appendDom(function(dom){
                                ...
                                ['add','uncheckboxIcon'].forEach(function(item){
                                    ...
                                    //若是add状态->需满足子项存在且长度>0
                                    (item!='add' || item=='add'&&curData[a].heeler&&curData[a].heeler.length>0) 
                                    &&
                                    ss.crtDom('div',item+'Dom',curSvg,dom,{
                                        ...
                                        cv:[
                                            'inline-block',self.scope['commonH']+'px',self.scope['commonH']+'px','relative','middle','2.5px',
                                            item=='add'?'3px':'5px'
                                        ],
                                        an:['type','ischeck','_index','_id'],
                                        av:[item , isTF?'true':'false',a,curData[a].id]
                                    },[
                                        'click',function(dom){
        ...
        lg_leaderCheckFn:function(dom){
            ...
            if(dom.getAttribute('ischeck')=='true'){
                ...
                obj.cliCbFn && obj.cliCbFn('false',dom,self);
            }
            else{
                //未点击
                ...
                obj.cliCbFn && obj.cliCbFn('true',dom,self);
            };
