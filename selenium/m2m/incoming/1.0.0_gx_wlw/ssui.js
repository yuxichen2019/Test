...
;!function(win){
    ...
    ss.prototype = {
        ...
        options:{
            privatePriMo:[
                'login','layout','menu','router','dataTable','dtView','dtPage','commonTool','dtVagueSle',
                'c3Loading','ajax','jsonData','ssView','ssRadio','blurrySel','ssSelect'
            ],//私有模块名称
        ...
        imports : function(para,callback){
            ...
            //string->单个模块依赖  ||  array->多个模块依赖
            else{
                var para = typeof para === 'string' ? [para] : para;
                onLoadMd(para);
                //轮询模块->加载函数
                var curMdStatus = self['options']['mdStatus'];
                var timer = setInterval(function(){
                    var isTF = true;//节流阀
                    for(var m=0; m<para.length; m++){
                        if(!curMdStatus[para[m]]){
                            isTF = false;
                            break;
                        }
                    };
                    if(isTF){
                        clearInterval(timer);
                        typeof callback == 'function' && callback(function(mdName,mdCode){
                            self[mdName] = mdCode;
                            self['options']['mdStatus'][mdName] = true;
                        })
                    }
                ...
                if(!self['options']['mdStatus'][para[0]]){
                    ...
                    node.addEventListener('load', function(e){
                        if (e.type === 'load' || (/^(complete|loaded)$/.test((e.currentTarget || e.srcElement).readyState))) {
                            ...
                            //判断参数长度
                            para.length>1 && onLoadMd(para.slice(1));
                        }
                    }, false);
                }
            };
        ...
        svgRepository : {
            ...
            //灰色未勾选->方框
            grayUnSel:function(count,colorVal){
                var colorVal = colorVal || '#bdbdbd';
                //fill='+colorVal+'>' +
                var tempStr = '<svg t="1509434054366" style="cursor: pointer;position:absolute;top:0px;left:0px;" viewBox="0 0 1024 1024" width='+count+' height='+count+'>' +
                    '<path d="M829.44 51.2H194.56C115.2 51.2 51.2 115.2 51.2 194.56v634.88C51.2 908.8 115.2 972.8 194.56 972.8h634.88c79.36 0 143.36-64 143.36-143.36V194.56C972.8 115.2 908.8 51.2 829.44 51.2zM417.28 760.32l-235.52-238.08 99.84-89.6 140.8 140.8 337.92-312.32 84.48 99.84-427.52 399.36z" p-id="3186" fill='+colorVal+'>' +
                    '</path>'+
                    '</svg>';
                return tempStr;
            },
